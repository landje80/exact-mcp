...
async function main() {
  ...
  const code = await new Promise<string>((resolve, reject) => {
    ...
    server.listen(port, () => {
      ...
      console.log();
      open(authUrl).catch(() => {
        // Browser could not be opened automatically — user can still click the link above.
      });
    });
  ...
  console.log();
  console.log("Klaar! Je kunt nu de MCP server starten (npm run build && npm start),");
